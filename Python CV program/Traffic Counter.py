...
roiWidth = (200, 1280)
roiHeight = (400, 500)
updatePeriod = 10  # seconds to update the server
updating = False
...
class Vehicle:
    def _init_(self):
        ...
        self.centerX = 0
        self.centerY = 0
        # new:just fonund       matched:matched to previous frame       unmatched:....
        self.state = "new"
        self.direction = 'up'  # or down
        self.name = randomString()
        self.pointsHistory = []    # centroid points history
        # as a blob by default it contains 1 vehicle    this could become more incase of two vehicles becoming one big blob
        self.contains = 1
...
def getClosest(v, ls, stat):
    ...
    for ov in ls:
        if ov.state == stat and closest.state != stat:
            closest = ov
        if v.distToOther(ov) < v.distToOther(closest) and ov.state == stat:
            closest = ov
    return closest
...
while ret:
    ...
    # fills the region with black color [0,0,0]
    dark[0:720, 0:1280] = ([0, 0, 0])
    # Blurred version of the frame
    blurred = cv2.GaussianBlur(frame, (35, 35), 0)
    # Mixed effect of blurr and tinted black
    darkBlurred = cv2.addWeighted(blurred, 0.3, dark, 0.7, 0)
    cv2.rectangle(darkBlurred, (roiWidth[0], roiHeight[0]), (
        roiWidth[1], roiHeight[1]), (0, 255, 200), 2)  # drawing a green Rect
    roi = frame[roiHeight[0]:roiHeight[1], roiWidth[0]:roiWidth[1]
                ]                                       # Region Of Interest
    ...
    darkBlurred[roiHeight[1]+10:roiHeight[1]+10 +
                (roiHeight[1]-roiHeight[0]), roiWidth[0]:roiWidth[1]] = bgrMask

    # -----  Blob Detection  -------
    contours, _ = cv2.findContours(
        carMask, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for c in contours:
        ...
        x, y, w, h = rect
        if w < 50 or h < 50:
            continue   # too small, probably a Pedestrian
        if w > 450 or h > 120:
            # too big, probably the first frame [which is just black then suddenly an image appears]
            continue
        # --------------------
    ...
    for v in newVehicles:
        if len(Vehicles) < 1:
            ...
            if v.distToOther(closest) < trackingThreshold:
                ...
                if len(v.pointsHistory) > 10:
                    v.pointsHistory.pop(0)
        ...
        if v.state == "unmatched":
            ...
            if v.distToOther(closest) < intersectionThreshold and closest.state == "new":
                if newCount == 2:       # seperation
                    if v.directionChanged == True and isCloseToEdge(v, 25):
                        ...
                        if v.direction == "up":
                            ...
                            downCarsCount -= 1
                # intersection, however there should be 2 close "unmatched"
                if newCount == 1 and countClosest(v, Vehicles, "unmatched", intersectionThreshold) == 2:
                    if closest.directionChanged != True and v.directionChanged == True:
                        closest.directionChanged = True
                ...
                if isCloseToEdge(v, 10):    # out of frame
                    ...
                    if v.direction == "up":
                        upCarsCount += v.contains
                    else:
                        downCarsCount += v.contains
            Vehicles.remove(v)
            i -= 1
        i += 1
    ...
    # ------ Draw
    for v in Vehicles:
        cv2.rectangle(darkBlurred, (v.x, v.y),
                      (v.x+v.w, v.y+v.h), (255, 150, 0), 2)
        cv2.putText(darkBlurred, v.name + " " + v.direction,
                    (v.x+v.w+10, v.y+v.h), 0, 0.5, (210, 255, 240))
        cv2.putText(darkBlurred, "Contains " + str(v.contains),
                    (v.x, v.y-3), 0, 0.5, (210, 255, 240))
        cv2.putText(darkBlurred, " State: " + v.state,
                    (v.x+v.w-10, v.y-3), 0, 0.5, (210, 255, 240))
        # draw history points
        for pt in v.pointsHistory:
            cv2.circle(darkBlurred, (int(pt[0]), int(
                pt[1])), 2, (255, 150, 0), 2)

    currSec = int(datetime.now().strftime('%S'))
    if currSec % updatePeriod == 0:   # Every 20 seconds
        if updating != True:
            print('Posting... (' + str(upCarsCount) +
                  ', ' + str(downCarsCount) + ')')
    ...
    cv2.putText(darkBlurred, 'Detected Objects : ' + str(len(newVehicles)) +
                "  [ " + getNamesOfVehicles(newVehicles) + "]", (10, 30), 2, 0.7, (0, 255, 0))
    cv2.putText(darkBlurred, 'Current Vehicles : ' + str(len(Vehicles)) +
                "  [ " + getNamesOfVehicles(Vehicles) + "]", (10, 60), 2, 0.7, (0, 255, 0))
    cv2.putText(darkBlurred, 'Total Count : ' +
                str(totalCarsCount), (10, 90), 2, 0.7, (0, 255, 0))
    cv2.putText(darkBlurred, 'Down Count : ' + str(downCarsCount),
                (roiWidth[0]+5, roiHeight[0]-10), 2, 0.7, (0, 255, 0))
    cv2.putText(darkBlurred, 'Up Count: ' + str(upCarsCount),
                (roiWidth[1]-170, roiHeight[0]-10), 2, 0.7, (0, 255, 0))
